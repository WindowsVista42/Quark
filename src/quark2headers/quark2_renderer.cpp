...
void quark::renderer::internal::init_window() {
  ...
  platform::window = glfwCreateWindow(platform::window_w, platform::window_h, platform::window_name, 0, 0);
  ...
  glfwSetCursorPosCallback(platform::window, quark::renderer::internal::update_cursor_position);

  glfwGetFramebufferSize(platform::window, &window_w, &window_h);
}
...
void quark::renderer::internal::init_vulkan() {
  ...
  glfwCreateWindowSurface(instance, platform::window, 0, &surface);
...
void quark::renderer::internal::copy_staging_buffers_to_gpu() {
  AllocatedBuffer old_buffer = quark::renderer::internal::gpu_vertex_buffer;
  LinearAllocationTracker old_tracker = quark::renderer::internal::gpu_vertex_tracker;
...
void quark::renderer::internal::begin_forward_rendering() {
  ...
  color_clear.color.float32[0] = 0.0f;//PURE_BLACK[0];
  color_clear.color.float32[1] = 0.0f;//PURE_BLACK[1];
  color_clear.color.float32[2] = 0.0f;//PURE_BLACK[2];
  color_clear.color.float32[3] = 0.0f;//PURE_BLACK[3];
...
void render_frame(bool end_forward) {
  ...
  {
    auto shadow_pass = ecs::registry.view<Position, Rotation, Scale, Mesh, UseShadowPass>();
  ...
  {
    auto depth_prepass = ecs::registry.view<Position, Rotation, Scale, Mesh>();
  ...
  {
    begin_solid_pass();
    auto solid_pass = ecs::registry.view<Position, Rotation, Scale, Mesh, Color, UseSolidPass>();
    ...
    auto wireframe_pass = ecs::registry.view<Position, Rotation, Scale, Mesh, Color, UseWireframePass>();
    ...
    begin_lit_pass();
    auto lit_pass = ecs::registry.view<Position, Rotation, Scale, Mesh, UseLitPass>();
...
void quark::renderer::internal::framebuffer_resize_callback(GLFWwindow* window, int width, int height) {
